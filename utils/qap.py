...
class QAP():
    ...
    def correlation(self, x, y, show=True):
        ...
        if show:
            print('Pearson Correlation: ', pc[0])
            print('p-value: ', pc[1])
        return pc
    ...
    def summary(self):
        ...
        utils.printf('')
        #print(f'prop >= {}: {}'.format(self.beta[0], sum([1 for b in self.betas if b >= self.beta[0] ])/float(len(self.betas))))
        #print(f'prop <= {}: {} (proportion of randomly generated correlations that were as {} as the observed)'.format(self.beta[0], sum([1 for b in self.betas if b <= self.beta[0] ])/float(len(self.betas)), 'large' if self.beta[0] >= 0 else 'small'))
        print('')
    ...
    def stats(self, x, y):
        if not self.diagonal:
            ...
            print(f'Z-Test: {ztest(xflatten, yflatten)}')
            print('T-Test: ', ttest_ind(xflatten, yflatten))
        else:
            p = np.corrcoef(x, y)
            print('Pearson\'s correlation:\n ', p)
            print('Z-Test: ', ztest(x, y))
            print('T-Test: ', ttest_ind(x, y))
